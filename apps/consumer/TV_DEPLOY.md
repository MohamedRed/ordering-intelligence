...
- Auth/Pay: Pairing via QR → mobile app deep link for OAuth + Stripe.
...
- Auth/Pay: Same QR/deep-link pairing to mobile for OAuth + Stripe; avoid card entry on TV.
...
- Auth/Pay: QR to mobile deep link; no direct card input.
...
- Skeleton lives in `apps/consumer/roku-channel/` (SceneGraph + BrightScript).
  - Current state: pairing QR screen only; replace QR URL per env and add icons/background before submission.
  - Auth/Pay: QR → mobile deep link to complete OAuth + Stripe; channel stores a short‑lived session token from channel-gateway.
  - APIs: reuse channel-gateway and consumer_core endpoints; expose a thin JSON endpoint for Roku payloads (menus, reorders, cart submit).
