...
class AgentCustomizationApi {
  ...
  Future<void> setSessionVoice({
    ...
    final resp = await _client.post(
      Uri.parse('$_agentCustomizationBaseUrl/v1/onboarding-sessions/$sessionId/voice'),
      headers: {
      ...
      body: jsonEncode({
        'voice_id': voiceId,
        if (voiceName != null && voiceName.isNotEmpty) 'voice_name': voiceName,
      }),
    );
    if (resp.statusCode != 200) {
      throw Exception('Set voice failed (${resp.statusCode})');
    }
  }

  Future<Map<String, dynamic>> getSessionVoice(String sessionId) async {
    final token = await _token();
    final resp = await _client.get(
      Uri.parse('$_agentCustomizationBaseUrl/v1/onboarding-sessions/$sessionId/voice'),
      headers: {
  ...
  Future<void> applySessionVoice({
    required String sessionId,
    String? voiceId,
  }) async {
    final token = await _token();
    final resp = await _client.post(
      Uri.parse('$_agentCustomizationBaseUrl/v1/onboarding-sessions/$sessionId/voice/apply'),
      headers: {
        'Content-Type': 'application/json',
        if (token != null) 'Authorization': 'Bearer $token',
      },
