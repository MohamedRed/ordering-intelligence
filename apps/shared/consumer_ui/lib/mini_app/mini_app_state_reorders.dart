...
mixin MiniAppStateReorders
    on State<MiniAppScreen>, MiniAppStateFields, MiniAppStateMenu, MiniAppStateCart {
  void _applyRecommendedOrder(RecommendedOrder order) {
    ...
    MiniAppScope.of(context).haptics.impact(style: 'light');
    ...
    if (added > 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Added $added item${added == 1 ? '' : 's'} to cart')),
      );
    }
    if (missing > 0) {
      ScaffoldMessenger.of(context).showSnackBar(
