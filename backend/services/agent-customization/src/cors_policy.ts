...
export function resolveCorsOrigins(rawOrigins: string | undefined, environment: string): string[] {
  ...
  if (!origins.length) {
    throw new Error('CORS_ORIGINS is required for agent-customization.');
  }

  if (isProductionLike(environment) && origins.includes('*')) {
    throw new Error('Wildcard CORS origins are not allowed for agent-customization in staging/production.');
  }
