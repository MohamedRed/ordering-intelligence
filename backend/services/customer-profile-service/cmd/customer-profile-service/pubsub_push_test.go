...
func TestDecodePubSubPushJSON(t *testing.T) {
	...
	reason, err := decodePubSubPushJSON(pubsubOrderBody(orderEvent{ID: "order-1"}), &order)
	if err != nil {
...
func TestHandleOrdersEventsAcknowledgesMalformedPubSubMessages(t *testing.T) {
	req := httptest.NewRequest(http.MethodPost, "/tasks/orders-events", strings.NewReader(`{"message":{}}`))
	rec := httptest.NewRecorder()

	handleOrdersEvents(nil)(rec, req)
